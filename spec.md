...
### 1-7-2. 지표의 필터는 값이 어디서 오는지를 함께 적는다

`fixed_filters` 라는 이름 하나에 성격이 다른 것들이 섞이기 쉽습니다. `source` 로 구분하십시오.

| `source` | 값 | 질문이 같은 컬럼을 건드리면 | 쓰임 |
|---|---|---|---|
| `literal` (기본) | `"ACTIVE"` | **그대로 AND** | 불변 조건 |
| `relative` | `"-7d"` | 물러남 | 최근 일주일 같은 이동 구간 |
| `question` | 없음 | 물러남 | 값을 질문에서 받는다 |

**`literal` 만 1-7 절이 말하는 강제 대상입니다.** `STATUS_CD='ACTIVE'` 가 빠지면 그것은 다른
지표이므로 모델이 뭘 하든 붙습니다. 반면 기간은 지표의 정체성이 아니라 질문의 매개변수입니다 —
`relative`·`question` 을 그대로 AND 로 이으면 이런 SQL 이 조용히 나갑니다.

```sql
-- 지표는 9월 일주일, 질문은 7월  →  교집합이 공집합
WHERE d >= DATE '2026-09-20' AND d < DATE '2026-09-28'
  AND d >= DATE '2026-07-01' AND d < DATE '2026-07-08'
```

**날짜를 `literal` 로 박아두지 마십시오.** 다음 주가 되면 그 지표는 지난주를 가리킵니다.
"최근 일주일"은 `relative` 로 적어야 뜻이 보존됩니다.

`relative` 는 `-7d` / `-1m` / `-1y` / `0d` 만 받고, 컴파일 시점에 **계산된 날짜**로 펴십시오
(`>= DATE '2026-09-16'`). `SYSDATE-7` 같은 DB 함수로 남기면 드라이버마다 문법이 갈리고, 읽는
사람이 어느 구간인지 알 수 없습니다. 달의 말일은 보정해야 합니다 — 3월 31일의 `-1m` 은 2월 28일입니다.

`question` 인데 질문이 그 컬럼을 지정하지 않으면 **거부하십시오.** 조건을 빼고 전체를 조회하면
큰 표에서 사고가 납니다.
...
- `fixed_filters[].source`가 `literal` | `relative` | `question` 중 하나인가 (1-7-2절)
- `literal`이고 날짜 컬럼이면 `value`가 `YYYY-MM-DD` 또는 `YYYY-MM-DD HH:MM:SS` 이고
  달력상 존재하는 날짜인가 (9절)
- `relative`이면 `value`가 `-7d` 형식이고, 그 컬럼이 날짜 컬럼인가
